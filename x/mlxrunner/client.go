...
// statusWriter captures the last short stderr line from the subprocess
// while forwarding all output to os.Stderr. Lines longer than maxStatusLen
// are skipped to avoid storing large prompts or data blobs.
...
func (w *statusWriter) Write(b []byte) (int, error) {
	...
	for {
		...
		w.buf = w.buf[i+1:]
		if len(line) > 0 && len(line) <= maxStatusLen {
			w.lastErrMsg = string(line)
		}
	}
	// if the buffer grows past maxStatusLen without a newline, discard it
