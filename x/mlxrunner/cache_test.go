	...
	forEachEnv(t, func(t *testing.T, env *testEnv) {
		...
		walkNodes(kvc.root, func(n *trieNode) bool {
			for i, s := range n.snapshots {
				if s != nil {
					ss := s.(*fakeSnapshot)
					ss.byteSize = 2 * 1024 * 1024 * 1024 // 2 GiB per snapshot
					n.snapshots[i] = ss
				}
			}
			return true
		...
		// System prompt prefix should still be findable (multi-child branch
		// point is ineligible for eviction).
