...
func (c *kvCache) switchToPath(newPath []*trieNode) {
	...
	if leaf := len(c.activePath) - 1; leaf >= commonLen {
		...
		if !node.hasAllSnapshots() {
			...
			node.snapshots = make([]cache.Snapshot, len(c.caches))
			for j, kv := range c.caches {
				if kv == nil {
					continue
				}
				node.snapshots[j] = kv.Snapshot(fromOffset)
			}
			pageOutCount++
...
func (c *kvCache) advancePath(frontier *trieNode, tokens []int32, endOffset int) *trieNode {
	...
	if len(matchPath) > 1 {
		...
		if matchedInEdge > 0 && matchedInEdge < len(lastNode.tokens) {
			split := splitNode(lastNode, matchedInEdge, c.caches)
			matchPath[len(matchPath)-1] = split
		}
	}
	...
	if len(remaining) > 0 {
		...
		if len(dest.children) == 0 && !dest.user {
			cache.CloseSnapshots(dest.snapshots)
			dest.snapshots = nil
		}
...
func (s *cacheSession) attachSnapshots(node *trieNode, cacheOffset int) {
	...
	for i, kv := range c.caches {
		if kv != nil {
			...
			node.snapshots[i] = kv.Snapshot(node.startOffset())
		}
	}
...
func (c *kvCache) clear() {
	...
	walkNodes(c.root, func(n *trieNode) bool {
		cache.CloseSnapshots(n.snapshots)
		n.snapshots = nil
		return true
	})
	c.root = nil
...
// enforceEvictionPolicy evicts LRU-eligible nodes until paged-out memory is within limits.
...
// pagedOutBytes returns the total heap bytes used by paged-out snapshots in the trie.
func (c *kvCache) pagedOutBytes() int64 {
	var total int64
	walkNodes(c.root, func(n *trieNode) bool {
...
// evictOne evicts the single LRU-eligible node. Returns true if a node was evicted.
func (c *kvCache) evictOne() bool {
	var candidate *trieNode
	var candidateTime time.Time

	// Build set of active path nodes for quick lookup.
	...
	walkNodes(c.root, func(n *trieNode) bool {
		if n == c.root {
			return true // never evict root
		}
		if activeSet[n] {
			return true // skip active path
		}

		// Only leaves and single-child interior nodes are eligible.
		// Multi-child interior nodes are skipped.
		if len(n.children) > 1 {
			return true
		}

		if candidate == nil || n.lastUsed.Before(candidateTime) {
			candidate = n
			candidateTime = n.lastUsed
		}
		return true
	})
	...
	// Calculate memory freed by this eviction.
	var freedBytes int64
	for _, s := range candidate.snapshots {
		if s != nil {
			freedBytes += int64(s.Size())
		}
	}
	...
	if len(candidate.children) == 0 {
		...
		slog.Debug(kind, "offset", candidate.startOffset(), "tokens", len(candidate.tokens), "freed", mlx.PrettyBytes(int(freedBytes)))
		...
		if parent != nil && !parent.user && len(parent.children) == 1 && parent != c.root {
			...
			mergeWithChild(parent, c.caches)
		}
	} else {
		// Interior snapshot node with one child: merge with child.
		slog.Debug("evicting snapshot node", "offset", candidate.endOffset, "tokens", len(candidate.tokens), "freed", mlx.PrettyBytes(int(freedBytes)))
		mergeWithChild(candidate, c.caches)
	}
