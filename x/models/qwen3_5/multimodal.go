...
func promptVisionMergedWidth(grid *VisionGrid, merge int32) int32 {
	if grid == nil {
		return 1
	}
	if merge <= 0 {
	...
	return max(int32(1), grid.Width/merge)
}
...
func (m *Model) buildPromptMRoPEPositions(state *promptVisionState, startPos, chunkLen int32) [4][]int32 {
	...
	for _, span := range state.Spans {
		...
		w := promptVisionMergedWidth(span.Grid, merge)
...
func (m *Model) tokenizePromptWithImages(
	prompt string,
	images []base.ImageInput,
	imageTokenCount func([]byte) (int32, error),
) ([]int32, *promptVisionState, error) {
	if m == nil || m.tok == nil {
		return nil, nil, fmt.Errorf("qwen3_5: tokenizer not initialized")
	}

	if m.Vision == nil || m.ImageProcessor == nil || imageTokenCount == nil {
	...
	for i, part := range parts {
		...
		tokensPerImage, err := imageTokenCount(img.Data)
		if err != nil {
			return nil, nil, err
		}
		...
		p += promptVisionGridSpan(nil, 1, tokensPerImage)
...
// TokenizePromptWithImages expands [img-n] tags into Qwen3.5 vision tokens.
func (m *Model) TokenizePromptWithImages(prompt string, images []base.ImageInput) ([]int32, error) {
	tokens, _, err := m.tokenizePromptWithResolvedImages(prompt, images, m.EncodeVisionImage)
	return tokens, err
}
