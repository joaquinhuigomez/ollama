...
func (p *VisionImageProcessor) ProcessImage(img image.Image) (*mlx.Array, *VisionGrid, error) {
	...
	patches, err := p.createPatches(pixels, resizedH, resizedW, grid)
	if err != nil {
		return nil, nil, err
	}
...
func (p *VisionImageProcessor) createPatches(pixels []float32, height, width int, grid *VisionGrid) ([]float32, error) {
...
func visionRotateHalf(x *mlx.Array) *mlx.Array {
	shape := x.Dims()
...
func applyVisionRoPE(x, cos, sin *mlx.Array) *mlx.Array {
	return mlx.Add(mlx.Mul(x, cos), mlx.Mul(visionRotateHalf(x), sin))
}
